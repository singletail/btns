...
static void mmwave_led(void) {
    if (pin_led)
        pin_write(pin_led, bits_chk(BIT_PRESENCE_DETECTED));
}
...
void mmwave_init(void) {
    if (!pin_sensor) {
        return;}
    bits_wait(BIT_SENSOR_OK);
    pin_config_in(
        pin_sensor, false, false, GPIO_INTR_ANYEDGE, mmwave_callback);
    if (pin_led)
        pin_config_out(PIN_MMWAVE_LED, false, false, GPIO_INTR_DISABLE);
    mmwave_read();
}
#endif