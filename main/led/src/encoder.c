...
static const float TICK_TO_US =
    RES_HZ / 1000000; // 80.0; // 80 ticks per microsecond
...
static size_t encoder_encode(rmt_encoder_t *encoder,
...
esp_err_t encoder_new(rmt_encoder_handle_t *ret_encoder) {
    ...
    // led_encoder = calloc(1, sizeof(encoder_obj_t));
    NONULL(led_encoder);
