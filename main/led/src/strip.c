...
DMA_ATTR static uint8_t
    strip_buffer_main_calc[STRIP_LENGTH_1 * STRIP_NUM_COLORS]
    __attribute__((aligned(16)));
...
strip_t *strip_create(
    gpio_num_t pin, uint32_t size, bool dma, strip_type_t type) {
    ...
    NOERR(rmt_new_tx_channel(&rmt_config, &strip->channel));
    NOERR(encoder_new(&strip->encoder));
    strips[type] = *strip;
    return strip;
}

esp_err_t strip_set_pixel_rgb(
    strip_t *strip, uint32_t pixel, uint8_t r, uint8_t g, uint8_t b) {
...
esp_err_t strip_set_range_rgb(strip_t *strip, uint32_t pixel_start,
    uint32_t pixel_end, uint8_t r, uint8_t g, uint8_t b) {
...
esp_err_t strip_enable(strip_t *strip) {
    NOERR(rmt_enable(strip->channel));
    return ESP_OK;
}

esp_err_t strip_disable(strip_t *strip) {
    NOERR(rmt_disable(strip->channel));
    return ESP_OK;
}
...
esp_err_t strip_refresh(strip_t *strip) {
    ...
    NOERR(rmt_enable(strip->channel));
    NOERR(rmt_transmit(strip->channel, strip->encoder, strip->buffer,
        strip->size * strip_conf.num_colors, &strip_tx_conf));
    NOERR(rmt_tx_wait_all_done(strip->channel, portMAX_DELAY));
    NOERR(rmt_disable(strip->channel));
...
esp_err_t strip_del(strip_t *strip) {
    NOERR(rmt_del_encoder(strip->encoder));
    NOERR(rmt_del_channel(strip->channel));
    free(strip);
    return ESP_OK;
}
...
void strip_cycle_task(void *arg) {
    ...
    timex_perf_t perf = timex_perf_new("strip_cycle");
    // timex_perf_add_led(&perf, PIN_SEG7_CLK, PIN_SEG7_DIO);
    uint8_t cycle_r = 0;
    uint8_t cycle_g = 0;
    uint8_t cycle_b = 0;
...
void strip_wave_task(void *arg) {
    ...
    float phases[6] = {0.0f, phase_offset, 2.0f * phase_offset,
        3.0f * phase_offset, 4.0f * phase_offset, 5.0f * phase_offset};
...
void strip_chase_task(void *arg) {
    ...
    strip_t *strip = (strip_t *)arg;
    timex_perf_t perf = timex_perf_new("strip_chase");
