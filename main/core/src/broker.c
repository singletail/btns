...
static esp_mqtt_client_handle_t broker_client;
...
void broker_status(esp_err_t status) {
    (status == ESP_OK) ? bits_on(BIT_MQTT_OK)
                       : bits_off(BIT_MQTT_OK);
}

esp_err_t chk(int msg_id) {
    esp_err_t err_rc_;
    (msg_id > -1) ? err_rc_ = ESP_OK : ESP_FAIL;
    broker_status(err_rc_);
    return err_rc_;
}

static const esp_mqtt_client_config_t broker_config = {
...
static void broker_event_handler(void *handler_args, esp_event_base_t base,
...
void broker_init(void) {
    ...
    if (broker_client != NULL) {
        esp_mqtt_client_start(broker_client);
        return;
    }
    broker_client = esp_mqtt_client_init(&broker_config);
    if (broker_client == NULL) {
        WARN("esp_mqtt_client_init() failed");
        return;
    }
    esp_mqtt_client_register_event(
        broker_client, ESP_EVENT_ANY_ID, broker_event_handler, NULL);
    esp_mqtt_client_start(broker_client);
}

void broker_connected(void) {
    LOG("mqtt connected to server.");
    chk(esp_mqtt_client_publish(
        broker_client, topic_status, "online", 0, 0, 1));
    chk(esp_mqtt_client_subscribe_single(broker_client, topic_subscribe, 0));
}

int broker_publish(
    const char *topic, const char *payload, int qos, int retain) {
    bits_wait(BIT_MQTT_OK);
    return chk(esp_mqtt_client_publish(
        broker_client, topic, payload, 0, qos, retain));
}
...
void broker_error(esp_mqtt_event_t *event) {
    ...
    if (error_type > 0) {
        ...
        case MQTT_ERROR_TYPE_TCP_TRANSPORT:
            ERR("reported from esp-tls",
                event->error_handle->esp_tls_last_esp_err);
            ERR("reported from tls stack",
...
void broker_subscribe(const char *topic) {
    ...
    INFO("mqtt_subscribe()");
    int msg_id = esp_mqtt_client_subscribe_single(broker_client, topic, 0);
    if (msg_id < 0) {
