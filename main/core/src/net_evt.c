...
static void net_evt_wifi_unhandled(
    esp_event_base_t evt_base, int32_t evt_id, void *evt_data) {
    printf("%s[%s] Unhandled Wi-Fi event -- base: %s, id: %ld\n", col_v,
        sym_net, wifi_event_name(evt_id), evt_id);
}

static void net_evt_wifi_error(void *evt_data) {
    bits_off(BIT_NET_OK);
    bits_on(BIT_NET_FAIL);
    wifi_event_sta_disconnected_t *evt =
        (wifi_event_sta_disconnected_t *)evt_data;
    printf("%s[%s] WIFI_EVENT_STA_DISCONNECTED reason: %d%s\n", col_r, sym_net,
        evt->reason, col_reset);
}
...
void net_evt_wifi(
    ...
    case WIFI_EVENT_STA_START:
        bits_on(BIT_NET_INIT);
        break;
    case WIFI_EVENT_STA_DISCONNECTED:
        net_evt_wifi_error(evt_data);
        break;
    default:
        net_evt_wifi_unhandled(evt_base, evt_id, evt_data);
        break;
    }
}
...
void net_evt_add_handlers(esp_netif_t *s_sta_netif) {
    esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID,
        &net_evt_wifi, s_sta_netif, &instance_any_id);
    esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP,
        &net_evt_wifi_ip, s_sta_netif, &instance_got_ip);
}
